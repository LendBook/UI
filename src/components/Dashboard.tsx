...
export default function Dashboard() {
  ...
  const [openChangePairedPriceDialog, setOpenChangePairedPriceDialog] =
    useState(false);
  const [openChangeLimitPriceDialog, setOpenChangeLimitPriceDialog] =
    useState(false);
  ...
  const totalDepositsETH = userOrders
    .filter((order) => order.asset === "ETH")
    .reduce((sum, order) => sum + parseFloat(order.size), 0);
  const totalBorrowsETH = userBorrows
    .filter((borrow) => borrow.asset === "ETH")
    .reduce((sum, borrow) => sum + parseFloat(borrow.size), 0);

  const totalDepositsUSDC = userOrders
    .filter((order) => order.asset === "USDC")
    .reduce((sum, order) => sum + parseFloat(order.size), 0);
  const totalBorrowsUSDC = userBorrows
    .filter((borrow) => borrow.asset === "USDC")
    .reduce((sum, borrow) => sum + parseFloat(borrow.size), 0);
  ...
  const [openIncreaseSizeBorrowDialog, setOpenIncreaseSizeBorrowDialog] =
    useState(false);
  ...
  if (
    !isNaN(parseFloat(wethBalance)) &&
    !isNaN(parseFloat(usdcBalance)) &&
    !isNaN(totalBorrowsETH) &&
    !isNaN(totalBorrowsUSDC) &&
    !isNaN(priceETHUSD) &&
    !isNaN(priceUSDCUSD)
  ) {
    totalBalance = parseFloat(
      (
        parseFloat(wethBalance) * priceETHUSD +
        parseFloat(usdcBalance) * priceUSDCUSD
      ).toFixed(2)
    );
    totalBorrow = parseFloat(
      (totalBorrowsETH * priceETHUSD + totalBorrowsUSDC * priceUSDCUSD).toFixed(
        2
      )
    );
    totalDeposit = parseFloat(
      (
        totalDepositsETH * priceETHUSD +
        totalDepositsUSDC * priceUSDCUSD
      ).toFixed(2)
    );
  } else {
  ...
  const handleInceaseSizeBorrow = async (
    positionId: number,
    repaidQuantity: string
  ) => {
    try {
      await repay(positionId, repaidQuantity);
    } catch (error) {
  ...
  const handleSubmitIncreaseSize = async (
    newSize: string,
    price: string,
    pairedPrice: string,
    isBuyOrder: string,
    ...
    const loadUserData = async () => {
      if (address) {
        try {
          ...
          for (const id of depositIds) {
            ...
            if (!ethers.BigNumber.from(order.quantity).isZero()) {
              const formattedOrder = {
                ...
                size: Number(
                  ethers.utils.formatUnits(order.quantity, 18)
                ).toFixed(2),
                asset: order.isBuyOrder ? "USDC" : "ETH",
                nextLimitPrice: Number(
                  ethers.utils.formatUnits(order.price, 18)
                ).toFixed(2),
                pairedPrice: Number(
                  ethers.utils.formatUnits(order.pairedPrice, 18)
                ).toFixed(2),
          ...
          const borrowFromIds = await orderbookContract.getUserBorrowFromIds(
            address
          );
          let borrowsDetails = [];
          for (const id of borrowFromIds) {
            ...
            if (!ethers.BigNumber.from(borrow.borrowedAssets).isZero()) {
              const formattedBorrow = {
                ...
                asset: "ETH",
                size: Number(
                  ethers.utils.formatUnits(borrow.borrowedAssets, 18)
                ).toFixed(2),
                closingPrice: Number(ethers.utils.formatUnits(0, 18)).toFixed(
                  2
                ),
                apy: 0,
              };
          ...
          >
            ...
            >
              ...
              <Typography variant="subtitle1">
                $ {Math.round(countTotalDeposit).toLocaleString("en-US")}
              </Typography>
            </Box>
            ...
            >
              ...
              <Typography variant="subtitle1">
                $ {Math.round(countTotalBorrow).toLocaleString("en-US")}
              </Typography>
            </Box>
            <Box sx={{ width: "2px", background: "#34363e" }} />
            <Box
            ...
            >
              ...
              <Typography variant="subtitle1">
                $ {Math.round(countTotalBalance).toLocaleString("en-US")}
              </Typography>
            </Box>
            {/* <Box sx={{ width: '2px', background: '#34363e' }} /> */}
                                    ...
                                    <TableCell align="left">
                                      ...
                                      >
                                        <button
                                          ...
                                          onClick={() =>
                                            handleWithdraw(order.id, order.size)
                                          }
                                        >
                                          Withdraw
                                        </button>
                                        ...
                                        >
                                          ...
                                          <MenuItem
                                            onClick={
                                              handleOpenChangePairedPriceDialog
                                            }
                                          >
                                            Change Paired Price
                                          </MenuItem>
                                          <MenuItem
                                            onClick={
                                              handleOpenChangeLimitPriceDialog
                                            }
                                          >
                                            Change Limit Price
                                          </MenuItem>
                                        ...
                                        <Dialog
                                          open={openIncreaseSizeDialog}
                                          onClose={
                                            handleCloseIncreaseSizeDialog
                                          }
                                        >
                                          <DialogTitle
                                            style={{
                                        ...
                                        <Dialog
                                          open={openChangeLimitPriceDialog}
                                          onClose={
                                            handleCloseChangeLimitPriceDialog
                                          }
                                        >
                                          <DialogTitle
                                            style={{
                                        ...
                                        <Dialog
                                          open={openChangePairedPriceDialog}
                                          onClose={
                                            handleCloseChangePairedPriceDialog
                                          }
                                        >
                                          <DialogTitle
                                            style={{
                                  ...
                                  >
                                    ...
                                    <TableCell align="left">
                                      ...
                                      >
                                        ...
                                        >
                                          <button
                                            className="btn btn-primary btn-sm bg-[#000000] mr-2"
                                            onClick={
                                              handleOpenIncreaseSizeBorrowDialog
                                            }
                                          >
                                            Increase
                                          </button>
                                        ...
                                        <Dialog
                                          open={openIncreaseSizeBorrowDialog}
                                          onClose={
                                            handleCloseIncreaseSizeBorrowDialog
                                          }
                                        >
                                          ...
                                          >
                                            ...
                                            <Button
                                              ...
                                              onClick={() =>
                                                handleSubmitIncreaseSizeBorrow()
                                              }
                                            >
                                              Increase Size
                                            </Button>
