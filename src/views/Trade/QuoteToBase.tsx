...
import { getMetricsDataTradeQuoteToBase } from "../../components/AnalyticsButtonsMetricLegend";
...
const QuoteToBase = () => {
  ...
  const [metricsData, setMetricsData] = useState(
    getMetricsDataTradeQuoteToBase(marketInfo)
  );

  const filteredData = orderMergedData.filter(
    (item) => item.pairedAvailableSupplyQuote !== 0
  );

  //take only the pools which are above price feed
  let refilteredData = filteredData.filter((item) => {
    if (typeof item.poolId === "number") {
      return item.poolId <= closestPoolIdUnderPriceFeed; //FIXEME need to be changed when we will deal with interest based liquidation
    }
    return false;
  });
  ...
  const handleRowClick = (rowData: any) => {
    ...
    const newAvailableSupply = rowData.pairedAvailableSupplyQuote;
      ...
      {sortedData.length !== 0 ? (
        <div className="flex justify-center mt-0 mb-15">
          <AnalyticsButtons
            ...
            specificMetric={"pairedAvailableSupplyQuote"}
          />
        </div>
      ) : (
      ...
      {buyPriceSelected && (
        <div className="flex justify-content align-items mt-10">
          <TradeBox
            poolId={poolId}
            sellToken="Quote"
            sellTokenName={marketInfo.quoteTokenSymbol}
            sellTokenRatioToUsd={1 ?? 0}
            buyTokenName={marketInfo.baseTokenSymbol}
            buyTokenRatioToUsd={price ?? 0}
            sellTokenMaxSupply={
            ...
            buyTokenMaxSupply={
              ...
              userInfo.quoteTokenBalance
                ...
                : userInfo.quoteTokenBalance * parseFloat(buyPrice)
            }
