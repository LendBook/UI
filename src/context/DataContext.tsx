...
import { useFetchUserInfo, UserDepositOrdersData, UserBorrowsData } from '../hooks/api/userInfo';
...
export const DataProvider: React.FC<DataProviderProps> = ({ children, provider, walletAddress }) => {
//  const { pricePoolId, pricePoolIdLoading, pricePoolIdError } = useFetchPriceForEmptyPools();
//  const { data: lendOrderData, loading: lendOrderLoading, error: lendOrderError } = useFetchLendOrder([1111111110]);
  ...
  const { userInfo, userDeposits, userBorrows, loadingUser, errorUser } = useFetchUserInfo(provider, walletAddress);
