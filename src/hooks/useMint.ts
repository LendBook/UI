...
export const useMint = (tokenContract: {
  connect: (arg0: ethers.providers.JsonRpcSigner) => {
    (): any;
    new (): any;
    mint: { (arg0: string, arg1: ethers.BigNumber): any; new (): any };
  };
}) => {
  ...
  return async (to: string, amount: string) => {
    ...
    try {
      ...
      const tx = await tokenContract.connect(signer).mint(to, parsedAmount);
      await tx.wait();
